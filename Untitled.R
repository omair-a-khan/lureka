MAPfreeze.list <- readRDS("/Users/khanoa/box/VMAC BIOSTAT/DATA/MAP/rawData/MAPfreeze_20201201.rds")
merged.df <- rVMAP::clear_labels(merged.df)
# names(merged.df) <- stringr::str_replace_all(names(merged.df), "\\.", "\\_")

np.var <- Hmisc::Cs(
  map_id,
  vmac_id,
  age,
  education,
  np_date,
  np_examiner,
  ethnicity,
  race,
  sex,
  np_moca,
  np_cvlt1,
  np_cvlt2,
  np_cvlt3,
  np_cvlt4,
  np_cvlt5,
  np_cvlt1z,
  np_cvlt2z,
  np_cvlt3z,
  np_cvlt4z,
  np_cvlt5z,
  np_cvlt1to5_tscore,
  np_cvltb,
  np_cvlt_sdfr,
  np_cvlt_sdcr,
  np_cvlt_ldfr,
  np_cvlt_ldcr,
  np_cvltrec_hits,
  np_cvltrec_falsepos,
  np_cvlt_reps,
  np_cvlt_intrus,
  np_cvltbz,
  np_cvlt_sdfr_z,
  np_cvlt_sdcr_z,
  np_cvlt_ldfr_z,
  np_cvlt_ldcr_z,
  np_cvltrec_hits_z,
  np_cvltrecog_falsepos_z,
  np_cvlt_reps_z,
  np_cvlt_intrus_z,
  np_biber1,
  np_biber2,
  np_biber3,
  np_biber4,
  np_biber5,
  np_biberb,
  np_biber_sd,
  np_biber_ld,
  np_biber_hits,
  np_biber_falsealarms,
  np_biber_t1to5_persev,
  np_biber_t1to5_extra,
  np_bnt,
  paste0("np_anim_q", formatC(1:4, width = 1, flag = "0")),
  np_anim_tscore,
  paste0("gds", formatC(1:30, width = 2, flag = "0")),
  np_hvot,
  np_tmta,
  np_tmta_seqerr,
  np_tmta_seterr,
  np_tmtb,
  np_tmtb_seqerr,
  np_tmtb_seterr,
  np_color,
  np_word,
  np_inhibit,
  np_fas_f,
  np_fas_a,
  np_fas_s,
  np_fas_intrus,
  np_fas_rep,
  paste0("np_tower", formatC(1:9, width = 2, flag = "0")),
  np_tower_ruleviol,
  np_digsymb,
  np_mc1_time,
  np_mc2_time,
  np_mc3_time,
  np_mc4_time,
  np_mc5_time,
  np_mc6_time,
  np_mc7_time,
  np_mc8_time,
  np_mc1_omissions,
  np_mc2_omissions,
  np_mc3_omissions,
  np_mc4_omissions,
  np_mc5_omissions,
  np_mc6_omissions,
  np_mc7_omissions,
  np_mc8_omissions,
  np_mc1_falsepos,
  np_mc2_falsepos,
  np_mc3_falsepos,
  np_mc4_falsepos,
  np_mc5_falsepos,
  np_mc6_falsepos,
  np_mc7_falsepos,
  np_mc8_falsepos,
  np_mc_kaplan,
  np_mc_kaplan_ss,
  np_symbol,
  np_symbol_trials
)

test <- MAPfreeze.list$epoch_1$data$main %>%
  filter(
    map_id == "001" & epoch == "1"
  ) %>%
  select(
    all_of(np.vars)
  )


